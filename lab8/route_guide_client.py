...
def guide_record_route(stub):
    ...
    print("Saved route with ID: %d" % route_summary.route_id)
...
def guide_route_retrieve(stub):
    print("Retrieving route for ID 0")
    route = stub.RouteRetrieve(route_guide_pb2.RouteId(id=0))
    for point in route:
        print("Point: %s" % format_point(point))

    print("Retrieving route for invalid ID 99")
    route = stub.RouteRetrieve(route_guide_pb2.RouteId(id=99))
    if not any(route):
        print("Server returned no points for invalid ID")
